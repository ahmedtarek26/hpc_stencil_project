...
#include <time.h>  // Added for time(NULL) in srand48
		 ...
		 )
{
  ...
  *output_energy_at_steps = 0;
    ...
    switch (opt) {
      ...
      case 'p':
        *periodic = atoi(optarg);
        break;
      case 'o':
        *output_energy_at_steps = atoi(optarg);
        break;
      case 'h':
  ...
  if (S[0] <= 0 || S[1] <= 0) {
    printf("Error: Grid sizes must be positive integers.\n");
    return 1;
  }
  if (*Nsources <= 0 || *Nsources > S[0] * S[1]) {
    printf("Error: Number of sources must be positive and <= grid size.\n");
    return 1;
  }
  ...
  if (*periodic != 0 && *periodic != 1) {
    printf("Error: Periodic flag must be 0 or 1.\n");
    return 1;
  }
  if (*output_energy_at_steps != 0 && *output_energy_at_steps != 1) {
    printf("Error: Output flag must be 0 or 1.\n");
    return 1;
  }
  if (*injection_frequency <= 0) {
    printf("Error: Injection frequency must be positive.\n");
    return 1;
  }
  ...
  //
  int ret = memory_allocate( S, planes );
 ...
 */
{
  ...
  unsigned int bytes = (size[_x_] + 2) * (size[_y_] + 2) * sizeof(double);

  planes_ptr[OLD] = (double*)malloc(2 * bytes);
  if (planes_ptr[OLD] == NULL) {
    perror("malloc failed");
    return 1;
  }
  memset(planes_ptr[OLD], 0, 2 * bytes);
  planes_ptr[NEW] = planes_ptr[OLD] + (size[_x_] + 2) * (size[_y_] + 2);
  ...
  if ( (filename != NULL) && (filename[0] != '\0') )
      ...
      for ( int j = 0; j < size[1]; j++ )
	{
	  ...
	  */
	  ...
	  const double * restrict line = data + (j + 1) * (size[0] + 2) + 1;
